...
| **`coldesc`** | 09-25 | **2.1 列描述注入**（baseline + BIRD 列说明注释） | 249 | **50.2%** | 98.0% | 0.107 | 净提升 0：翻对 13、翻错 13（p=1.0）；成本 ×1.75 |
...
- 单题输入 token 619 → 1863（×3.0），输出 1016 → 850；缓存命中 67% → 83%；
  **单题成本 0.061 → 0.107（×1.75）**，总成本 26.6 credit
...
单独做 2.1 不划算。两个后续线索：过时格式可以靠 2.2 的样例值纠正（模型能直接看到真实取值）；
