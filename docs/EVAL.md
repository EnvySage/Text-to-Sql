...
**成本**：网关按 credit 计价，而**单价会变**：同样的 token，09-20 每千输出 token 扣 0.060 credit，
09-25 扣 0.091（涨约 50%）。

- 跨日期**不能**直接比 credit；比较成本时，要么和同一时段跑的对照比，要么比 token 数
- 结果文件里的 token 数不受计价变化影响，是成本比较的主依据
...
| **`coldesc`** | 09-25 | **2.1 列描述注入**（baseline + BIRD 列说明注释） | 249 | **50.2%** | 98.0% | 0.107 | 与 baseline 差 0、与同期重跑差 +2.8，都在噪声内；同期成本 ×1.17 |
| **`baseline-rerun`** | 09-25 | **噪声测量**：baseline 原样重跑 | 249 | 47.4% | 98.0% | 0.091 | 同配置两次差 2.8 个点、25 题翻转；成本比 09-20 高 49%，是网关计价变了 |
...
- 单题成本 0.107。~~相对 baseline ×1.75~~：那是跨日期比较，网关计价在 09-20 到 09-25 之间涨了约 50%（见 4.1 节）。
  和同期的 `baseline-rerun`（0.091）比是 **×1.17**
